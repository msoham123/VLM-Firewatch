...
class Moondream2FineTuner:
    ...
    def lr_schedule(self, step: int) -> float:
        ...
        if x < 0.1:
            ...
            return self.base_lr * (x / 0.1)  # ← FIX
        else:
            # Cosine decay
            return 0.1 * self.base_lr + 0.9 * self.base_lr * (1 + math.cos(math.pi * (x - 0.1) / 0.9)) / 2
...
def main():
    ...
    parser.add_argument("--learning_rate", type=float, default=1e-4, help="Learning rate")  # ← CHANGED
