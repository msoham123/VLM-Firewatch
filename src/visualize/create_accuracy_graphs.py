...
# Quantized MoonDream2 data
moondream_quantized = {
    'base': [0.8961509854964671, 0.7747920665387076, 0.9899456073842097],
    'quantized': [0.998977314986984, 0.9978673491149499, 0.9998351738915444]
}
...
# Create fourth graph: MoonDream2 Quantized vs Base
fig, ax = plt.subplots(figsize=(10, 6))
...
# Get data for quantized comparison
base_accuracies = [acc * 100 for acc in moondream_quantized['base']]
quantized_accuracies = [acc * 100 for acc in moondream_quantized['quantized']]

# Find min and max for reasonable y-axis range
all_values = base_accuracies + quantized_accuracies
min_val = min(all_values)
max_val = max(all_values)
...
# Create bars
bars1 = ax.bar(x - width/2, base_accuracies, width, label='Base', color='#3498db', alpha=0.8)
bars2 = ax.bar(x + width/2, quantized_accuracies, width, label='Quantized', color='#2ecc71', alpha=0.8)

# Customize the plot
ax.set_xlabel('Category', fontsize=12, fontweight='bold')
ax.set_ylabel('Accuracy (%)', fontsize=12, fontweight='bold')
ax.set_title('Fire Detection Accuracy for MoonDream2: Base vs Quantized', fontsize=14, fontweight='bold')
ax.set_xticks(x)
ax.set_xticklabels(categories)
ax.legend(fontsize=10)
ax.set_ylim(y_min, y_max)
...
# Adjust layout and save
plt.tight_layout()
plt.savefig('graphs/MoonDream2_Quantized_vs_Base.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"✓ Created graph for MoonDream2 Quantized vs Base")
